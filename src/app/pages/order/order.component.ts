...
export class OrderComponent {
  ...
  paymentOrder() {
    ...
    let data: any = {
      user: this.userLogin._id,
      totalPrice: this.calculatePrice(),
      paymentMethod: this.paymentMethodChecked,
    };

    if (this.paymentMethodChecked === "Thanh toán khi nhận hàng" && this.checkedVoucher) {
      data.vouchers = [this.checkedVoucher._id];
    }

    if (this.paymentMethodChecked === "Thanh toán khi nhận hàng") {
      this.orderSevice.createOrder(data).subscribe((response) => {
